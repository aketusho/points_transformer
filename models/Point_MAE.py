...
from .pointnet2_utils import sample
...
#         """
#         KNN Indexing
#         input: nsample: int32, Number of neighbor 9 （K=9）
...
#         if new_xyz is None:
#             new_xyz = xyz
#         xyz = xyz.contiguous() #(3,1024,3)
#         new_xyz = new_xyz.contiguous() #(3,1024,3)
#         assert xyz.is_contiguous()
#         assert new_xyz.is_contiguous()
#         b, m, _ = new_xyz.size() #b:3, m:1024
#         n = xyz.size(1)#1024
#         idx = torch.cuda.IntTensor(b, m, nsample).zero_() #(3,1024,9) 建了一个空表
#         dist2 = torch.cuda.FloatTensor(b, m, nsample).zero_() #(3,1024,9) 建了一个空表
...
class KNNQuery(nn.Module):
    def __init__(self,):
        super().__init__()
        self.k=27;
    def forward(self, nsample, xyz, new_xyz) :
        """
        KNN Indexing
        input: nsample: int32, Number of neighbor 9 （K=9）
               xyz: (b, n, 3) coordinates of the features
...
def query_knn_point(k, xyz, new_xyz, cuda=False):
    knnquery = KNNQuery()
    if cuda:
        if not xyz.is_contiguous(): #True
            xyz = xyz.contiguous()
        if not new_xyz.is_contiguous(): #True
            new_xyz = new_xyz.contiguous()
        return knnquery(k, xyz, new_xyz) #得到idex(3,1024,9)
...
def nonanneigbor(center, neigborhood):
    for b in range(center.size(0)):
        for i in range(center.size(1)):
            #small = 9999999999
            for j in range(neigborhood.size(2)):
                #print("iffffffffffffff")
                if torch.any(torch.not_equal(neigborhood[b][i][j], center[b][i])) == True:
                    #print("neigborhood[b][i][j]", neigborhood[b][i][j])
                    #print("center[b][i]", center[b][i])#两个点不完全相等的情况
                    #dist1 = torch.nn.functional.pairwise_distance(neigborhood[b][i][j], center[b][i], p=2, keepdim=True)
                    #print("dist1", dist1)
                    #if dist1 < small:
                    p = neigborhood[b][i][j]
                        #small = dist1
            for j in range(neigborhood.size(2)): #32
                if torch.any(torch.not_equal(neigborhood[b][i][j], center[b][i])) == False: #两个点完全一样
                    neigborhood[b][i][j] = p
    return neigborhood
...
class Group(nn.Module):  # FPS + KNN
    ...
    def forward(self, xyz, is_print=False):
        ...
        if is_print:
            ...
            print("input point shape:", xyz.shape);
        # fps the centers out
        #center = misc.fps(xyz, self.num_group) # B G 3
        center = sample(self.num_group, xyz)
        if is_print:
            print("input center:",center);
        # knn to get the neighborhood
        #_, idx = self.knn(xyz, center) # B G M
        print("idx.shape")
        idx = query_knn_point(self.group_size, xyz, center, cuda=True)
        print("idx.shape:", idx.shape)#(b, 128, 32)
        if is_print:
            print("chose idx:",idx);
        ...
        neighborhood = nonanneigbor(center, neighborhood)
        if is_print:
...
class PointTransformer(nn.Module):
    ...
    def forward(self, pts, num=5):
        ...
        neighborhood, center = self.group_divider(pts);#center(b,128,3) neighborhood(b,128,32,3)
        ...
        print("after transformer blocks:", x);
